...
class Dying(pygame.sprite.Group):
    ...
    def died(self, unit):
        if random.randint(0, 48) == 0:
            unit.image = pygame.transform.scale(settings.IMAGE_LOADER.power_up, (int(0.618 * unit.size[0]), int(0.618 * unit.size[1])))
            unit.power_up = True
        else:
            unit.image = pygame.transform.scale(settings.IMAGE_LOADER.ink_stain, unit.size)
